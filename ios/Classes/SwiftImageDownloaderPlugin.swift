...
public class SwiftImageDownloaderPlugin: NSObject, FlutterPlugin {
    ...
    public static func register(with registrar: FlutterPluginRegistrar) {
        let channel = FlutterMethodChannel(
            name: "plugins.ko2ic.com/image_downloader",
    ...
    public func handle(
        ...
        case "downloadImage":
            ...
            guard let dict = call.arguments as? [String: Any?],
                ...
                result(
                    FlutterError(
                        code: "assertion_error", message: "url is required.",
                        details: nil))
                return
            }
        ...
        case "open":
            ...
            break
        case "findPath":
            setData(call, result: result) { id in
                findPath(id) { value in result(value) }
            }
        case "findName":
            setData(call, result: result) { id in result(findName(id)) }
        case "findByteSize":
            setData(call, result: result) { id in
                findByteSize(id) { value in result(value) }
            }
        case "findMimeType":
            setData(call, result: result) { id in
                findMimeType(id) { value in result(value) }
            }
        default:
    ...
    private func setData(
        ...
        guard let id = (call.arguments as? [String: Any?])?["id"] else {
            result(
                FlutterError(
                    code: "assertion_error", message: "id is required.",
                    details: nil
                )
            )
            return
    ...
    private func saveImage(_ data: Data, result: @escaping FlutterResult) {
        guard let image = UIImage(data: data) else {
            result(
                FlutterError(
                    code: "data_error",
                    message: "Couldn't convert to UIImage.",
                    details: nil
                )
            )
            return
        ...
        if let ref = CGImageSourceCreateWithData(data as CFData, nil) {
            if let metadata = CGImageSourceCopyPropertiesAtIndex(ref, 0, nil)
                as? [String: AnyObject]
            {
                if let exifDict = metadata["{Exif}"] as? [CFString: AnyObject] {
                    exifDatetime =
                        exifDict[kCGImagePropertyExifDateTimeOriginal]
                        as? String
                }
                ...
                if let tiffDict = metadata["{TIFF}"] as? [CFString: AnyObject] {
                    if let datetime = tiffDict[kCGImagePropertyTIFFDateTime]
                        as? String
                    {
        ...
        PHPhotoLibrary.shared().performChanges({
            ...
            if let imageId = imageId, isSuccess {
                ...
                {
                    guard
                        let imageUrl = self.createTemporaryFile(
                            fileName: nil,
                            ext: "jpg"
                        )
                    else {
                    ...
                    result(
                        FlutterError(
                            ...
                            message: "Couldn't save to photo library.",
                            details: detail
                        )
                    )
                    return
    ...
    private func saveGif(
        data: Data, name: String?, result: @escaping FlutterResult
    ) {
        DispatchQueue.main.async {
            _ = UIImageView(image: UIImage(data: data))
        }

        guard let imageUrl = createTemporaryFile(fileName: name, ext: "gif")
        else {
            return
        }
        do {
            ...
            result(
                FlutterError(
                    code: "save_error",
                    message: "Couldn't save gif to photo library.",
                    details: error
                )
            )
        }
    ...
    private func saveVideo(
        ...
        guard let videoUrl = createTemporaryFile(fileName: name, ext: "mp4")
        else {
            return
        }
            ...
            PHPhotoLibrary.shared().performChanges(
                ...
                completionHandler: { isSuccess, error in
                    if let imageId = imageId, isSuccess {
                        ...
                        {
                            let detail = [
                                "unsupported_file_path": videoUrl.path
                            ]
                            result(
                                FlutterError(
                                    code: "unsupported_file",
    ...
    private func createTemporaryFile(fileName: String?, ext: String) -> URL? {
        ...
        let fileName =
            (fileName != nil)
            ? fileName! : "\(formatter.string(from: now)).\(ext)"
        guard
            let temporaryFile = NSURL(
            ...
            result(
                FlutterError(
                    code: "save_error",
                    message: "Couldn't save \(ext) in temporary directory.",
                    details: nil
                )
            )
            return nil
    ...
    private func open(
        ...
        guard let path = (call.arguments as? [String: Any?])?["path"] else {
            result(
                FlutterError(
                    code: "assertion_error", message: "path is required.",
                    details: nil
                )
            )
            return
        ...
        if !controller.presentPreview(animated: true) {
            result(
                FlutterError(
                    code: "preview_error",
                    message: "This file is not supported for previewing",
                    details: nil))
        }
    }
    ...
    private func findPath(
        ...
        if asset.mediaType == PHAssetMediaType.video {
            PHImageManager().requestAVAsset(
                forVideo: asset, options: nil,
                resultHandler: { avurlAsset, _, _ in
    ...
    private func findName(_ id: String) -> String {
        return PHAsset.fetchAssets(
            withLocalIdentifiers: [id], options: nil
        ).firstObject?.value(forKey: "filename") as! String
    }

    private func findByteSize(
        _ id: String,
            ...
            PHImageManager().requestAVAsset(
                ...
                resultHandler: { avurlAsset, _, _ in
                    ...
                    if let uti = UTTypeCreatePreferredIdentifierForTag(
                        kUTTagClassFilenameExtension, pathExtension as NSString,
...
extension SwiftImageDownloaderPlugin: URLSessionDataDelegate {
    ...
    public func urlSession(
        ...
        let percentageDownloaded =
            Float(fileData.length) / Float(expectedContentLength)
        progress = percentageDownloaded

        channel.invokeMethod(
            "onProgressUpdate",
            arguments: ["progress": Int(progress * 100)]
        )
    }
}
...
extension SwiftImageDownloaderPlugin: URLSessionDelegate {
    public func urlSession(
        _: URLSession, task downloadTask: URLSessionTask,
        ...
        if error != nil {
            ...
            result(
                FlutterError(
                    code: "request_error", message: error?.localizedDescription,
                    details: error
                )
            )
            result = nil
        ...
        if let statusCode = httpResponse?.statusCode {
            if 400...599 ~= statusCode {
                result(
                    FlutterError(
                        code: "\(statusCode)",
                        message: "HTTP status code error.", details: nil
                    )
                )
                result = nil
                return
        ...
        case 0x47:
            ...
            saveGif(
                data: data,
                name: (downloadTask.response as? HTTPURLResponse)?
                    .suggestedFilename, result: result)
        case 0x49, 0x4D:
            // image/tiff
            saveImage(data, result: result)
        case 0x00:
            if data.count >= 12 {
                let testString = String(
                    data: data.subdata(in: 4..<12), encoding: .nonLossyASCII)
                // image/heic
                    ...
                    || testString == "ftyphevx"
                {
                    ...
                    let newImage = UIImage(
                        cgImage: uiImage.cgImage!, scale: 1.0,
                        orientation:
                            MetaDataUtils.getNormalizedUIImageOrientation(
                                CGImagePropertyOrientation(
                                    rawValue: orientation)))
                    let jpegData = newImage.jpegData(compressionQuality: 1.0)!
                    let newData = MetaDataUtils.updateMesataData(
                        metaData: metadata, imageData: jpegData)
                    ...
                    if let contentType = contentType {
                        if contentType.contains("vide")
                            || contentType == "application/octet-stream"
                        {
                            saveVideo(
                                data: data,
                                name: httpResponse?.suggestedFilename,
                                result: result)
                            return
                        }
                    }
